...
元素流在管道中经过中间操作（intermediate operation）的处理，最后由终结操作(terminal operation)得到前面处理的结果。



**必须有终结操作(terminal operation), 否则中间操作不会被调用到**
...
`Arrays.stream(数组)`或者`Stream.of(数组)`
...
class Test {
  public static void main(String[] args) {
    ...
    Stream<Integer> stream2 = Stream.of(arr); // 此处不能直接对基本数据类型的数组使用
  }
}
```
...
class Test {
  void test() {
      int[] arr = {1, 3, 4, 56, 3, 2, 20, 12};
      Arrays.stream(arr).filter(i -> i < 10).forEach(System.out::println); // 过滤掉大于10的元素
  }
}
```
...
和`map()`方法的区别在于, 这个方法用于将元素中包含的集合转换成流返回
...
| min()      | 返回最小值 |
| collect()  | 将流转换成集合 |
| anyMatch() | 是否有元素符合匹配规则 |
| allMatch() | 是否所有元素都符合匹配规则 |
| findAny() | 获取流中的任意一个元素, 返回Optional对象 |
| findFirst() | 获取流中的第一个元素, 返回Optional对象 |
| reduce() | 归并操作, 对流中的数据按照指定的计算方式计算出一个结果 (缩减操作), 相当于元素和或者元素乘积 |
...
1. 必须有终结操作, 否则中间操作不会执行
2. 流是一次性的, 流在经过终结操作后会关闭
3. 不会影响原数据
